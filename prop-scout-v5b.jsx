...
// ─────────────────────────────────────────────
const SlateCard = ({ game, selected, onSelect }) => {
    ...
    <div onClick={() => onSelect(game.id)} style={{ background: selected ? "rgba(34,197,94,0.06)" : "#161827", border: `1px solid ${selected ? "rgba(34,197,94,0.4)" : "#1f2437"}`, borderRadius: 12, padding: "12px", cursor: "pointer", marginBottom: 8, transition: "all 0.15s" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
        ...
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 11, color: "#f9fafb", fontWeight: 700 }}>O/U {game.odds.total}</div>
          <div style={{ fontSize: 10, color: game.odds.lineMove === "over" ? "#f59e0b" : game.odds.lineMove === "under" ? "#38bdf8" : "#6b7280", marginTop: 2 }}>
...
export default function App() {
  ...
  const game = SLATE.find(g => g.id === selectedId);
  const { pitcher, batter, props, umpire, odds, nrfi, bullpen } = game;
    ...
    <>
      ...
      <div style={{ background: "#0e0f1a", minHeight: "100vh", color: "#e5e7eb", fontFamily: "monospace", maxWidth: 480, margin: "0 auto", padding: "16px 14px 48px" }}>
        ...
        {view === "slate" && (<>
          ...
          {SLATE.map(g => (
            <SlateCard key={g.id} game={g} selected={selectedId === g.id} onSelect={openGame} />
          ))}
        </>)}
        ...
        {view === "game" && (<>
          ...
          {tab === "intel" && (<>
            ...
            {/* Line Movement */}
            <SLabel>Odds &amp; Line Movement</SLabel>
            <Card>
              <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
        ...
        <div style={{ fontSize: 10, color: "#374151", textAlign: "center", marginTop: 10, lineHeight: 1.8 }}>
          ⚠ Demo mode — mock data · Live API connects in production
        </div>
      </div>
    </>
  );
